...
export default function BlogLoading() {
  return (
    <main className="flex flex-1 flex-col">
      {/* Hero skeleton — mirrors the brand-red PageHero so there's no visual jump on transition. */}
      <section className="bg-brand-red py-10 md:py-14" aria-hidden>
        <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="bg-white/15 mb-4 h-3 w-32 rounded" />
          <div className="bg-white/25 h-8 w-48 rounded md:h-10 md:w-64" />
          ...
          {Array.from({ length: 6 }).map((_, i) => (
            ...
            >
              ...
              <div className="space-y-3 p-5">
                ...
                <div className="bg-cream-dark/40 h-3 w-5/6 rounded" />
              </div>
            </div>
          ))}
        </div>
