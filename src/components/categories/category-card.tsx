  ...
  return (
    ...
    >
      ...
      {couponCount !== undefined && couponCount > 0 && (
        <span className="font-accent bg-brand-gold text-cream absolute top-2 start-2 rounded-full px-2 py-0.5 text-xs font-bold leading-none shadow-sm">
          {couponCount}
        </span>
      )}

      <div className="bg-brand-red/10 text-brand-red group-hover:bg-brand-red group-hover:text-cream flex h-14 w-14 items-center justify-center rounded-2xl transition-all duration-200">
        <Icon className="h-7 w-7" strokeWidth={1.75} aria-hidden />
      </div>
      <span className="font-display text-charcoal group-hover:text-brand-red text-center text-sm font-bold transition-colors">
        {category.name_ar}
      </span>
    </Link>
  );
